...
function checkDuplicates(array, prop) {
...
function getValidUpdWD(wd_arr_upd, wd_arr_upd_from_db) {
    let valid_wd_arr = [];
    wd_arr_upd.forEach(item => {
        ...
        const wd_item = getValidWD(new_item);
        if (wd_item) {
...
function getValidAddWD(wd_arr_add) {
    let valid_wd_arr = [];
    wd_arr_add.forEach(item => {
        const wd_item = getValidWD(item);
        if (wd_item) {
            valid_wd_arr.push(wd_item);
        }
    });
...
function getValidWD(wd_item) {
    const wtbObj = getTimeObj(wd_item.wd_time_begin);
    const wteObj = getTimeObj(wd_item.wd_time_end);
    ...
    if (wtbObj && wteObj) {
        ...
        if (wtbObj.timeMinute < wteObj.timeMinute &&
            ...
            if (wd_item.wd_break_begin && wd_item.wd_break_end) {
                ...
                const wbbObj = getTimeObj(wd_item.wd_break_begin);
                const wbeObj = getTimeObj(wd_item.wd_break_end);
...
function getRecordsForOneWD(wd) {
    let rec_arr = [];
    // Создать список wd_id, rec_time
    const wtbObj = getTimeObj(wd.wd_time_begin);
    const wteObj = getTimeObj(wd.wd_time_end);
    ...
    if (wd.wd_break_begin && wd.wd_break_end) {
        const wbbObj = getTimeObj(wd.wd_break_begin);
        const wbeObj = getTimeObj(wd.wd_break_end);
...
function getRecordsForWDs(wd_arr) {
    let rec_arr = [];
    wd_arr.forEach(item => {
        // Создать список wd_id, rec_time
        const wtbObj = getTimeObj(item.wd_time_begin);
        const wteObj = getTimeObj(item.wd_time_end);
        ...
        if (item.wd_break_begin && item.wd_break_end) {
            const wbbObj = getTimeObj(item.wd_break_begin);
            const wbeObj = getTimeObj(item.wd_break_end);
...
function getWdWithRecForUpd(wd_arr_valid, wd_with_rec) {
    let result = [];
    wd_arr_valid.forEach(item => {
        // 1. Создаем таймпоинты;
        let rec_arr = getRecordsForOneWD(item);
        // 2. Проверяем стыковку записей;
        if (wd_with_rec.every(i => {
            return !(i.wd_id === item.wd_id &&
                !rec_arr.some(rec => {
                    return getTimeObj(rec.rec_time).timeMinute === getTimeObj(i.rec_time).timeMinute;
                }));
        })) {
            ...
            wd_with_rec.forEach(i => {
                if (i.wd_id === item.wd_id) {
                    for (let k = 0; k < rec_arr.length; k++) {
                        if (getTimeObj(rec_arr[k].rec_time).timeMinute === getTimeObj(i.rec_time).timeMinute) {
                            rec_arr.splice(k, 1);
                            k--;
                        }
                    }
...
exports.updWorkingDays = (knex, emp_id, wd_arr_upd) => {
    return new Promise((resolve, reject) => {
        ...
        if (checkDuplicates(wd_arr_upd, "wd_id")) {
        ...
        knex.transaction((trx) => {
            return knex({wd: T.WORKING_DAYS.NAME, rec: T.RECORDS.NAME})
                ...
                .then((res) => {
                    ...
                    wd_arr_upd_without_rec = getValidUpdWD(wd_arr_upd, res);
                ...
                .then((res) => {
                    ...
                    let rec_arr = getRecordsForWDs(wd_arr_upd_without_rec);
                ...
                .then((res) => {
                    ...
                    let wd_with_rec_WDonly = removeDuplicates(wd_with_rec.map(item => {
                        return {
                            wd_id: item.wd_id,
                            emp_id: item.emp_id,
                    ...
                    let validUpdWD = getValidUpdWD(wd_arr_upd, wd_with_rec_WDonly);
                    ...
                    wd_with_rec = getWdWithRecForUpd(validUpdWD, wd_with_rec);
...
exports.insertWorkingDays = (knex, emp_id, wd_arr_add) => {
    return new Promise((resolve, reject) => {
        ...
        if (checkDuplicates(wd_arr_add, "wd_date")) {
        ...
        let cdObj = new dateClass(currentDate.getUTCFullYear(),
            currentDate.getUTCMonth() + 1, currentDate.getUTCDate());
        for (let i = 0; i < wd_arr_add.length; i++) {
            const dt = getDateObj(wd_arr_add[i].wd_date);
        ...
        knex.transaction((trx) => {
            ...
            const validAddWD = getValidAddWD(wd_arr_add);
            ...
            return knex(T.WORKING_DAYS.NAME)
                ...
                .then((res) => {
                    if (res.length !== 0) {
                        for (let i = 0; i < validAddWD.length; i++) {
                            if (res.some(item => {
                                const validDT = getDateObj(validAddWD[i].wd_date);
                                const itemDT = new dateClass(getDateString(item.wd_date));
                ...
                .then((res) => {
                    ...
                    const rec_arr = getRecordsForWDs(res);
                    return knex(T.RECORDS.NAME)
                        .transacting(trx)
                        .insert(rec_arr)
                        .returning(T.RECORDS.WD_ID);
                ...
                .then((res) => {
                    ...
                    result = added_wd.map(item => {
                        return getDateString(item.wd_date);
                    });
                    resolve(result);
                })
                .then(() => {
